...
packages <- c("ggplot2", "cowplot", "viridisLite", "sjPlot", "grid")
...
#This code shows there are duplicate BIRD_KEYS/ID due to multiple measures of plastic weight and count due to bolus, lavage and necropsy sources

#tab_df(head(mass_df))
#tab_df(table(mass_df$ID))

#Find all rows with duplicate BIRD_KEY and add those weights and numbers together.
## One row per bird: sum numeric columns within ID + BIRD_KEY

# 1) Pick the numeric columns to sum
is_num <- sapply(mass_df, is.numeric)

# 2) Summariser that keeps NA if everything is NA in a group
sum_keep_na <- function(x) {
  if (all(is.na(x))) return(NA_real_)
  sum(x, na.rm = TRUE)
}

# 3) Aggregate: sums for numeric columns, grouped by ID + BIRD_KEY
mass_by_bird <- aggregate(
...
#Storing original dataset and replacing the aggregated dataset
mass_df_store <- mass_df
mass_df <- mass_by_bird
...
if (!exists("pal_group")) {
  pal_group <- c("Low plastic (≤0.5)" = "#0072B2",
...
dfA <- within(mass_df, {
  Total_mass <- suppressWarnings(as.numeric(Total_mass))
  Wt         <- suppressWarnings(as.numeric(Wt))
})
dfA <- dfA[is.finite(dfA$Total_mass) & is.finite(dfA$Wt), , drop = FALSE]
dfA$plastic_group <- factor(
  ifelse(dfA$Total_mass > 0.5, "High plastic (>0.5)", "Low plastic (≤0.5)"),
...
pB_core <- ggplot(agg_s2, aes(x = time_min, y = mean, colour = plastic_group, fill = plastic_group)) +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks = element_line(linewidth = 0.5),
        plot.margin  = margin(2, 2, 2, 2))
pB_noleg <- pB_core + guides(colour = "none", fill = "none") + theme(legend.position = "none")
...
pC_core <- ggplot(agg_relax2, aes(x = time_min, y = mean, colour = plastic_group, fill = plastic_group)) +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks = element_line(linewidth = 0.5),
        plot.margin  = margin(2, 2, 2, 2))
pC_noleg <- pC_core + guides(colour = "none", fill = "none") + theme(legend.position = "none")

# ---------- Panel D (Legend only) ----------
# Build a dedicated legend plot, centred, with title on top to reduce width
...
pD <- ggplot(legend_df, aes(x, y, colour = plastic_group)) +
  # invisible line so the legend shows line keys; override to make them visible in the key
...
fig_ABCD <- cowplot::plot_grid(top_row, bot_row, nrow = 2, rel_heights = c(1, 1))

# Save
dev_fun <- if (capabilities("cairo")) cairo_pdf else "pdf"
ggsave("figure_ABCD_square.pdf", fig_ABCD, device = dev_fun,
