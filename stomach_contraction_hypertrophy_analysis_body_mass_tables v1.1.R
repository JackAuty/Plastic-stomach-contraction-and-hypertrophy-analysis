...
# ============================================================
# Export body-weight-adjusted GAMM results as HTML tables
# ============================================================

model_table_dir <- file.path(DATA_DIR, "GAMM_model_tables")
dir.create(model_table_dir, showWarnings = FALSE, recursive = TRUE)

# Contraction models
sjPlot::tab_model(
  res_con_body_mass$m0$gam,
  res_con_body_mass$m1$gam,
  title = "Contraction GAMMs adjusted for bird body weight",
  dv.labels = c(
    "Reduced model: body weight and common time curve",
    "Full model: body weight, plastic group and group-specific curves"
  ),
  auto.label = FALSE,
  show.intercept = TRUE,
  show.est = TRUE,
...
# Relaxation models
sjPlot::tab_model(
  res_rel_body_mass$m0$gam,
  res_rel_body_mass$m1$gam,
  title = "Relaxation GAMMs adjusted for bird body weight",
  dv.labels = c(
    "Reduced model: body weight and common time curve",
    "Full model: body weight, plastic group and group-specific curves"
  ),
...
# Helper to extract the ML model-comparison statistics from the lme components
extract_gamm_comparison <- function(result, phase_label) {
  comparison <- anova(result$m0$lme, result$m1$lme)

  lrt <- if ("L.Ratio" %in% names(comparison)) {
    comparison[["L.Ratio"]]
  } else {
    rep(NA_real_, nrow(comparison))
  }

  p_value <- if ("p-value" %in% names(comparison)) {
    comparison[["p-value"]]
  } else {
    rep(NA_real_, nrow(comparison))
  }
  ...
  data.frame(
    Phase = phase_label,
    Model = c(
      "Reduced: body weight and common time curve",
      "Full: body weight, plastic group and group-specific curves"
    ),
    df = comparison$df,
    AIC = comparison$AIC,
    BIC = comparison$BIC,
...
model_comparisons <- rbind(
  extract_gamm_comparison(res_con_body_mass, "Contraction"),
  extract_gamm_comparison(res_rel_body_mass, "Relaxation")
)

# Round displayed statistics while retaining small p-values
model_comparisons$AIC <- round(model_comparisons$AIC, 2)
model_comparisons$BIC <- round(model_comparisons$BIC, 2)
model_comparisons$logLik <- round(model_comparisons$logLik, 2)
model_comparisons$Likelihood_ratio <- round(
  model_comparisons$Likelihood_ratio,
  3
)
model_comparisons$p_value <- ifelse(
  is.na(model_comparisons$p_value),
  "",
  ifelse(
    model_comparisons$p_value < 0.001,
    "<0.001",
    formatC(model_comparisons$p_value, format = "f", digits = 3)
  )
)

sjPlot::tab_df(
  model_comparisons,
  title = "Maximum-likelihood comparison of body-weight-adjusted GAMMs",
  col.header = c(
    "Phase",
    "Model",
    "df",
  ...
  digits = 3,
  alternate.rows = TRUE,
  file = file.path(model_table_dir, "GAMM_model_comparisons.html")
)
